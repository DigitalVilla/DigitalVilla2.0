...
(if [ -z "$1" ]; then
  ...
  # Script starts
    ...
    read -p "Would you like to destroy [$1]? Y/N " ANSWER
    echo ""

    case "$ANSWER" in
      [yY] | [yY][eE][sS])
        yarn app:remove $1 $2 $3 $5 $4 $5 $6 $7 $8 $9
        node scripts/nx/remove $1 --app
        ;;
      [nN] | [nN][oO])
  ...
  # Script ends
    ...
    echo "ERROR: Found suffix '-api', use [api:destroy] for APIs"
  fi
else
